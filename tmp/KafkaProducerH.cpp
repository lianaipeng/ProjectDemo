...
CKafkaProducer::CKafkaProducer(const string &brokers, const string &topics, int nPartition /*= 1*/) 
{
    m_bRun              = true; 
    m_strTopics         = topics; 
    m_strBroker         = brokers; 
    //m_nPartition        = nPartition;
    m_nPartition        = RD_KAFKA_PARTITION_UA;
...
#if 0 
#else
CKafkaProducer::err_cb(rd_kafka_t *rk, int err, const char *reason, void *opaque)
{
