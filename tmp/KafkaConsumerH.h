...
/*
 *  CKafkaConsumer *CKafkaConsumer_ = new CKafkaConsumer("localhost:9092", "test", "1", 0);
 *  if (!CKafkaConsumer_->Init()){
 *      fprintf(stderr, "kafka server initialize error\n");
 *  }
 *  else
 *  {
 *      CKafkaConsumer_->Start(1000);
 *  }
 */
...
{
public:
    CKafkaConsumer(char* brokers, char* topic, MessageCallBack pCallBack, void* pUser) {};
    CKafkaConsumer(const string& brokers, const string& topics, string groupid, int32_t nPartition = 0, int64_t offset = 0);
    ~CKafkaConsumer();
